-- Compatibility fix for databases where the newer availability migration has not yet been applied.
-- business_hours is the sole weekly schedule source. No fallback schedule is introduced.
-- The function deliberately does not require barber_services rows: the current setup UI
-- does not expose a service/barber assignment screen, so an absent assignment must not
-- make a correctly configured business hour invisible to the customer.
...
BEGIN
  ...
  -- PostgreSQL DOW is stable and timezone-independent for a DATE:
  ...
  -- If explicit service/barber links exist, respect them. If no links exist at all
  -- for this service, the setup UI's model treats the active barber as eligible.
  SELECT EXISTS (
    SELECT 1
