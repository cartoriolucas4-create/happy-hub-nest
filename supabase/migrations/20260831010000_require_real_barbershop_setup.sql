...
RETURNS BOOLEAN LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (SELECT 1 FROM public.services s WHERE s.barbershop_id = p_barbershop_id AND s.ativo AND length(trim(s.nome)) >= 2 AND s.preco >= 0 AND s.duracao_minutos > 0)
    AND EXISTS (SELECT 1 FROM public.barbers b WHERE b.barbershop_id = p_barbershop_id AND b.ativo AND length(trim(b.nome)) >= 2)
    AND EXISTS (SELECT 1 FROM public.business_hours h WHERE h.barbershop_id = p_barbershop_id AND h.aberto)
    AND NOT EXISTS (SELECT 1 FROM public.business_hours h WHERE h.barbershop_id = p_barbershop_id AND h.aberto AND (h.hora_inicio IS NULL OR h.hora_fim IS NULL OR h.hora_fim <= h.hora_inicio))
    AND EXISTS (SELECT 1 FROM public.payment_methods p WHERE p.barbershop_id = p_barbershop_id AND p.active);
$$;

CREATE OR REPLACE FUNCTION public.barbearia_operacional(p_barbershop_id UUID)
RETURNS BOOLEAN LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$ SELECT public.barbershop_setup_complete(p_barbershop_id); $$;
...
-- Both public booking signatures are guarded so an incomplete shop cannot be booked directly.
CREATE OR REPLACE FUNCTION public.criar_agendamento_publico(
  p_slug TEXT, p_service_id UUID, p_barber_id UUID, p_data DATE, p_hora TIME,
  p_nome TEXT, p_telefone TEXT, p_email TEXT DEFAULT NULL, p_observacao TEXT DEFAULT NULL
)
...
BEGIN
  ...
  RETURN public.criar_agendamento_publico(p_slug, p_service_id, p_barber_id, p_data, p_hora, p_nome, p_telefone, p_email, p_observacao, NULL::uuid);
END;
$$;
...
CREATE OR REPLACE FUNCTION public.criar_agendamento_publico(
  p_slug text, p_service_id uuid, p_barber_id uuid, p_data date, p_hora time without time zone,
  p_nome text, p_telefone text, p_email text DEFAULT NULL::text, p_observacao text DEFAULT NULL::text,
...
BEGIN
  ...
  VALUES (v_shop, v_customer, v_barber, p_service_id, trim(p_nome), trim(p_telefone), p_data, p_hora, v_end, v_preco, 'pendente', nullif(trim(coalesce(p_observacao, '')), ''), v_pm_id, v_pm_nome) RETURNING id INTO v_id;
  RETURN v_id;
