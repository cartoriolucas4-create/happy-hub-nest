...
BEGIN
  ...
  SELECT * INTO v_bh FROM public.business_hours
   WHERE barbershop_id = v_shop AND dia_semana = v_dow AND aberto = true;
  IF v_bh IS NULL THEN RETURN; END IF;

  SELECT EXISTS (
    SELECT 1 FROM public.barber_services bs
    JOIN public.barbers b ON b.id = bs.barber_id AND b.barbershop_id = v_shop AND b.ativo
    WHERE bs.service_id = p_service_id AND bs.barbershop_id = v_shop
  ...
  FOR v_b IN
    ...
    LEFT JOIN public.barber_hours bh
      ON bh.barber_id = b.id AND bh.dia_semana = v_dow AND bh.ativo = true
    WHERE b.barbershop_id = v_shop AND b.ativo = true
      ...
      AND (
        ...
        OR EXISTS (
          SELECT 1 FROM public.barber_services bs
          WHERE bs.barber_id = b.id AND bs.service_id = p_service_id AND bs.barbershop_id = v_shop
        )
      )
    ORDER BY b.nome
  LOOP
    ...
    WHILE v_slot + make_interval(mins => v_dur) <= LEAST(v_bh.hora_fim, COALESCE(v_b.h_fim, v_bh.hora_fim)) LOOP
      ...
      IF (p_data > v_today OR (p_data + v_slot) > v_now)
         AND NOT (
           ...
           AND v_bh.intervalo_inicio IS NOT NULL AND v_bh.intervalo_fim IS NOT NULL
           AND v_slot < v_bh.intervalo_fim AND v_end > v_bh.intervalo_inicio
         )
         AND NOT EXISTS (
           SELECT 1 FROM public.appointments a
           WHERE a.barbershop_id = v_shop AND a.barber_id = v_b.id AND a.data = p_data
             AND a.status <> 'cancelado' AND v_slot < a.hora_fim AND v_end > a.hora_inicio
         )
         AND NOT EXISTS (
