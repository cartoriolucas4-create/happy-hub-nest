-- business_hours uses intervalo_inicio/intervalo_fim only.
-- Never reference the nonexistent possui_intervalo column.
...
BEGIN
  ...
  SELECT s.duracao_minutos INTO v_duration
  FROM public.services s
  WHERE s.id = p_service_id AND s.barbershop_id = v_shop AND s.ativo = true
  LIMIT 1;
  IF v_duration IS NULL OR v_duration <= 0 THEN RETURN; END IF;

  v_dow := EXTRACT(DOW FROM p_data)::integer;

  SELECT bh.hora_inicio, bh.hora_fim, bh.intervalo_inicio, bh.intervalo_fim
  INTO v_start, v_end, v_break_start, v_break_end
  FROM public.business_hours bh
  WHERE bh.barbershop_id = v_shop AND bh.dia_semana = v_dow AND bh.aberto = true
  ORDER BY bh.id
  LIMIT 1;
  ...
  SELECT EXISTS (
    ...
    WHERE bs.barbershop_id = v_shop AND bs.service_id = p_service_id
  ) INTO v_has_service_links;

  SELECT b.id, b.nome INTO v_barber
  ...
  IF v_barber.id IS NULL THEN RETURN; END IF;
  ...
  WHILE v_slot + make_interval(mins => v_duration) <= v_end LOOP
    ...
    IF (p_data > v_today OR (p_data = v_today AND v_slot > v_now))
      AND NOT (
        v_break_start IS NOT NULL AND v_break_end IS NOT NULL
        AND v_slot < v_break_end AND v_slot_end > v_break_start
      )
      AND NOT EXISTS (
