...
function Servicos() {
  ...
  return (
    <AdminShell title="Serviços" subtitle="Preço e duração usados no agendamento online" actions={<button onClick={() => setForm(vazio)} className={btn}><span className="flex items-center gap-2"><Plus className="h-4 w-4" /> NOVO SERVIÇO</span></button>}>
      ...
      <div className="grid gap-3 sm:grid-cols-2">{(servicos ?? []).map((s) => <div key={s.id} className="rounded-lg border border-border bg-card p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="text-xl uppercase">{s.nome}</h3><p className="text-sm text-muted-foreground">{brl(s.preco)} · {s.duracao_minutos} min {s.ativo ? "" : "· inativo"}</p>{s.descricao && <p className="mt-2 text-sm text-muted-foreground">{s.descricao}</p>}</div><div className="flex gap-2"><button aria-label="Editar" onClick={() => setForm({ id: s.id, nome: s.nome, descricao: s.descricao ?? "", preco: String(s.preco), duracao_minutos: String(s.duracao_minutos), ativo: s.ativo })} className="text-muted-foreground hover:text-primary"><Pencil className="h-4 w-4" /></button><button aria-label="Excluir" disabled={excluir.isPending} onClick={() => { if (confirm(`Excluir definitivamente o serviço \"${s.nome}\"? Esta ação não pode ser desfeita.`)) excluir.mutate(s.id); }} className="text-muted-foreground hover:text-destructive disabled:opacity-50"><Trash2 className="h-4 w-4" /></button></div></div></div>)}</div>
    </AdminShell>
  );
}
