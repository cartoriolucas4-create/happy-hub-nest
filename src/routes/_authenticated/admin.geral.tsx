...
function AdminGeral() {
  ...
  async function salvar() {
    ...
    const { error } = await supabase
      .from("platform_settings")
      .upsert({ id: "default", support_whatsapp: digits || null, updated_at: new Date().toISOString() });
    setSaving(false);
    if (error) {
      ...
      <div className="max-w-2xl space-y-6">
        <section className="rounded-xl border border-border bg-card p-6 sm:p-8">
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-primary">Suporte</p>
              <h2 className="mt-1 text-2xl">WhatsApp da equipe</h2>
              <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                Esse número será usado pelo botão “Falar com a equipe” no Dashboard de todas as barbearias.
              </p>
            </div>
          </div>

          <label className="mt-8 block">
            <span className="text-xs uppercase tracking-widest text-muted-foreground">Número do WhatsApp</span>
            <input
            ...
            <span className="mt-2 block text-xs text-muted-foreground">
              Use código do país + DDD + número. Ex.: +55 82 99999-9999.
            </span>
          </label>

          <div className="mt-6 flex items-center justify-between gap-4 rounded-lg border border-border bg-background p-4">
            <div className="flex items-center gap-3">
              <Shield className="h-5 w-5 text-primary" />
              <div>
                <p className="text-sm font-medium">Mensagem automática</p>
                <p className="text-xs text-muted-foreground">Olá, sou /ID-DO-SITE, e tenho uma dúvida.</p>
              </div>
            </div>
          </div>

          <button type="button" className={`${btn} mt-6 inline-flex items-center gap-2`} onClick={salvar} disabled={loading || saving}>
