...
function MeuLink() {
  ...
  async function compartilhar() {
    if (!status?.completo || !shop) return;
    if (navigator.share) {
  ...
  return <AdminShell title="Meu link" subtitle="Divulgue no Instagram, WhatsApp e Google">
    {!shop || !status ? <p>Carregando...</p> : !status.completo ? <div className="max-w-2xl rounded-lg border border-primary/40 bg-primary/5 p-6">
      <div className="flex items-start gap-3"><AlertTriangle className="mt-0.5 h-5 w-5 text-primary"/><div><h2 className="font-display text-sm tracking-widest">SEU SITE AINDA NÃO ESTÁ PRONTO</h2><p className="mt-2 text-sm text-muted-foreground">Complete a configuração da sua barbearia antes de compartilhar seu link.</p></div></div>
      ...
      <div className="rounded-lg border border-border bg-card p-6"><p className="text-xs uppercase tracking-widest text-muted-foreground">Seu link público</p><p className="mt-2 break-all font-display text-2xl text-primary">{url}</p><div className="mt-6 flex flex-wrap gap-3"><button className={btn} onClick={copiar}><span className="flex items-center gap-2"><Copy className="h-4 w-4"/>COPIAR LINK</span></button><button className={btnGhost} onClick={compartilhar}><span className="flex items-center gap-2"><Share2 className="h-4 w-4"/>COMPARTILHAR</span></button><Link to="/barbearia/$slug" params={{ slug: shop.slug }} target="_blank" className={btnGhost}><span className="flex items-center gap-2"><ExternalLink className="h-4 w-4"/>ABRIR SITE</span></Link></div></div>
