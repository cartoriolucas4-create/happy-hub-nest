...
import { CalendarDays, Users, DollarSign, Clock, FileText } from "lucide-react";
...
function Dashboard() {
  ...
  const gerarRelatorio = async () => {
    ...
    try {
      ...
      if (reportStatus) query = query.eq("status", reportStatus as Status);
      else query = query.in("status", ["confirmado", "concluido"]);
      const { data: vendas, error } = await query;
      if (error) throw error;
      emitReportPdf(shop.nome, reportDe, reportAte, (vendas ?? []) as ReportAppointment[]);
      setReportOpen(false);
    } catch (error) {
      window.alert(error instanceof Error ? error.message : "Não foi possível gerar o relatório.");
    } finally {
  ...
  return <AdminShell title={shop?.nome ?? "Dashboard"} subtitle="Visão geral de hoje" actions={<div className="flex flex-wrap gap-2">{shop && <Link to="/admin/meu-link" className="rounded-md border border-border px-4 py-2 text-sm hover:border-primary hover:text-primary">/barbearia/{shop.slug}</Link>}<button className={`${btn} inline-flex items-center gap-2`} onClick={() => setReportOpen(true)}><FileText className="h-4 w-4" /> RELATÓRIO DE VENDAS</button></div>}>
    ...
    {reportOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" role="dialog" aria-modal="true" aria-labelledby="report-title">
      <div className="w-full max-w-xl rounded-lg border border-border bg-card p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-4"><div><h2 id="report-title" className="text-2xl">Relatório personalizado</h2><p className="mt-1 text-sm text-muted-foreground">Escolha o período e, se quiser, o status das vendas.</p></div><button className={btnGhost} onClick={() => setReportOpen(false)}>FECHAR</button></div>
        <div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="text-sm">De<input type="date" className={`${input} mt-1 w-full`} value={reportDe} onChange={(e) => setReportDe(e.target.value)} /></label><label className="text-sm">Até<input type="date" className={`${input} mt-1 w-full`} value={reportAte} onChange={(e) => setReportAte(e.target.value)} /></label><label className="text-sm sm:col-span-2">Status<select className={`${input} mt-1 w-full`} value={reportStatus} onChange={(e) => setReportStatus(e.target.value as "" | "confirmado" | "concluido")}><option value="">Confirmadas + concluídas</option><option value="confirmado">Somente confirmadas</option><option value="concluido">Somente concluídas</option></select></label></div>
        {reportDe > reportAte && <p className="mt-3 text-sm text-destructive">A data inicial não pode ser posterior à data final.</p>}
        <div className="mt-6 flex justify-end gap-2"><button className={btnGhost} onClick={() => setReportOpen(false)}>Cancelar</button><button className={`${btn} inline-flex items-center gap-2`} disabled={reportLoading || reportDe > reportAte} onClick={gerarRelatorio}><FileText className="h-4 w-4" />{reportLoading ? "GERANDO..." : "EMITIR PDF"}</button></div>
      </div>
    </div>}
  </AdminShell>;
}
...
function Card({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: string }) {
  return <div className="rounded-lg border border-border bg-card p-5 transition-colors hover:border-primary/60"><Icon className="h-5 w-5 text-primary" aria-hidden="true" /><p className="mt-3 text-xs uppercase tracking-widest text-muted-foreground">{label}</p><p className="mt-1 font-display text-3xl">{value}</p></div>;
}
