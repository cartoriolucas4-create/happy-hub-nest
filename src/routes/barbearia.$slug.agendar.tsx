...
import { ArrowLeft, CalendarDays, Check, Clock3, Copy, MessageCircle } from "lucide-react";
...
function Agendar() {
  ...
  useEffect(() => {
    if (!base) return;
    const firstOpen = Array.from({ length: 61 }, (_, i) => addDays(todayIso(), i)).find((iso) => {
  ...
  async function copiarPix() {
    if (!metodo?.pix_key) return;
    try { await navigator.clipboard.writeText(metodo.pix_key); toast.success("Chave Pix copiada!"); }
  ...
  const confirmar = useMutation({
    mutationFn: async () => {
      ...
      return waLink(base!.shop, mensagemAgendamento({
    ...
    onSuccess: (url) => { setWhatsUrl(url); setStep(7); if (url) window.open(url, "_blank", "noopener,noreferrer"); },
  ...
  const resumo: [string, string][] = [
    ["Barbearia", base.shop.nome], ["Serviço", servico?.nome ?? ""], ["Barbeiro", barbeiro?.nome ?? ""], ["Data", brDate(data)], ["Horário", hora],
    ["Duração", `${servico?.duracao_minutos ?? 0} minutos`], ["Valor", brl(servico?.preco ?? 0)], ["Cliente", nome.trim()], ["WhatsApp", telefone.trim()],
  ...
  if (step === 7) return (
    <Centro>
      ...
      <div className="mx-auto mt-8 max-w-md rounded-lg border border-border bg-card p-6 text-left text-sm">{resumo.map(([k, v]) => <p key={k} className="flex justify-between gap-4 border-b border-border/60 py-1.5 last:border-0"><span className="text-muted-foreground">{k}</span><span className="text-right">{v}</span></p>)}</div>
      {metodo && isPix(metodo.name) && metodo.pix_key && <div className="mx-auto mt-5 max-w-md rounded-lg border border-primary/30 bg-primary/5 p-5 text-left"><p className="text-xs uppercase tracking-widest text-primary">Pagamento via Pix</p><p className="mt-2 text-sm"><span className="text-muted-foreground">Beneficiário:</span> {metodo.pix_beneficiary || "Não informado"}</p><div className="mt-3 flex items-center justify-between gap-3 rounded-md border border-border bg-card p-3"><span className="min-w-0 break-all text-sm">{metodo.pix_key}</span><button type="button" onClick={copiarPix} className="inline-flex shrink-0 items-center gap-1 rounded-md border border-primary px-3 py-2 text-xs font-semibold text-primary hover:bg-primary hover:text-primary-foreground"><Copy className="h-3.5 w-3.5" />Copiar chave</button></div></div>}
      {whatsUrl && <a href={whatsUrl} target="_blank" rel="noopener noreferrer" className="mt-6 inline-flex items-center gap-2 rounded-sm bg-emerald-500 px-6 py-3 font-display tracking-widest text-white hover:bg-emerald-600"><MessageCircle className="h-5 w-5" /> ENVIAR NO WHATSAPP</a>}
      <div><Link to="/barbearia/$slug" params={{ slug }} className="mt-6 inline-block rounded-sm border border-border px-6 py-3 text-sm hover:border-primary hover:text-primary">Voltar para a barbearia</Link></div>
    </Centro>
  );
  ...
  return (
    <div className="min-h-screen bg-background px-5 py-10 text-foreground">
      <div className="mx-auto max-w-xl">
        <Link to="/barbearia/$slug" params={{ slug }} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-primary"><ArrowLeft className="h-4 w-4" /> {base.shop.nome}</Link>
        <div className="mt-6 flex gap-2">{[1, 2, 3, 4, 5, 6].map((n) => <div key={n} className={`h-1 flex-1 rounded ${n <= step ? "bg-primary" : "bg-secondary"}`} />)}</div>
