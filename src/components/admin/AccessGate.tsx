  ...
  return (
    <div className={`mb-8 flex flex-wrap items-center justify-between gap-4 rounded-lg border p-4 ${urgente ? "border-destructive/40 bg-destructive/10" : trial ? "border-primary/40 bg-primary/10" : "border-emerald-500/30 bg-emerald-500/10"}`}>
      <div className="flex items-start gap-3">
        ...
        <div><p className="font-display text-sm tracking-widest">{trial ? "TESTE GRATUITO" : "ACESSO ATIVO"}</p><p className="mt-1 text-sm text-muted-foreground">{trial ? "Seu acesso gratuito termina em " : "Seu acesso está ativo até "}<strong className="text-foreground">{trial ? formatarRestante(ms) : dataHoraBr(licenca.expires_at)}</strong>{!trial && <>{" · tempo restante "}<strong className="text-foreground">{formatarRestante(ms)}</strong></>}</p>{aviso && <p className="mt-1 text-xs text-muted-foreground">{aviso}</p>}</div>
      </div>
      <a href={whatsappSuporte(mensagem, supportWhatsapp)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-md border border-border px-4 py-2 text-xs tracking-widest hover:border-primary hover:text-primary"><MessageCircle className="h-4 w-4" aria-hidden="true" /> FALAR COM A EQUIPE</a>
    </div>
  );
}
...
export function AccessGate({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="mx-auto max-w-xl rounded-lg border border-border bg-card p-8 text-center">
      <Lock className="mx-auto h-8 w-8 text-destructive" aria-hidden="true" /><h2 className="mt-5 text-2xl">{bloqueado ? "ACESSO BLOQUEADO" : "SEU ACESSO EXPIROU"}</h2>
      ...
      <p className="mt-3 text-xs text-muted-foreground">Nenhum dado da sua barbearia foi excluído. Ao liberarmos o acesso, tudo continua disponível.</p><p className="mt-1 text-xs text-muted-foreground">Venceu em {dataHoraBr(licenca.expires_at)}</p>
      <div className="mt-7 flex flex-wrap items-center justify-center gap-3"><a href={whatsappSuporte(mensagem, supportWhatsapp)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-md bg-primary px-6 py-3 font-display text-sm tracking-widest text-primary-foreground hover:bg-primary/90"><MessageCircle className="h-4 w-4" aria-hidden="true" />{expirado && licenca.access_expires_at ? "RENOVAR ACESSO" : "ENTRAR EM CONTATO"}</a><button onClick={() => queryClient.invalidateQueries()} className="rounded-md border border-border px-5 py-3 text-sm hover:border-primary hover:text-primary">Já liberei, atualizar</button></div>
    </div>
  );
}
